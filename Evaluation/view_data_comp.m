...
load(sprintf('../Developing/%s/output_HR_%s_phase/pred_img_%s.mat',   model_name, load_dir, epoch_label_phase));
pred_img_phase  = pred_img;
load(sprintf('../Developing/%s/output_HR_%s_comp2/pred_img_%s.mat',   model_name, load_dir, epoch_comp2));
pred_img_comp2  = pred_img;
load(sprintf('../Developing/%s/output_HR_%s_comp/pred_img_%s.mat',   model_name, load_dir, epoch_comp));
pred_img_comp   = pred_img;
...
recon_mag_comp   = pred_img_comp(:,:,:,1);
recon_mag_comp   = permute(recon_mag_comp,   [2 3 1]);
recon_phase_comp = pred_img_comp(:,:,:,2);
recon_phase_comp = permute(recon_phase_comp, [2 3 1]);

recon_mag_comp2   = pred_img_comp2(:,:,:,1);
recon_mag_comp2  = permute(recon_mag_comp2,   [2 3 1]);
recon_phase_comp2 = pred_img_comp2(:,:,:,2);
recon_phase_comp2 = permute(recon_phase_comp2, [2 3 1]);

recon_phase_lp = pred_img_phase(:,:,:,1);
recon_phase_lp = permute(recon_phase_lp, [2 3 1]);
...
jimg(recon_mag_comp);
jimg(recon_phase_comp, [0 1]);

jimg(recon_mag_comp2);
jimg(recon_phase_comp2, [0 1]);

jimg(recon_phase_lp, [0 1]);
...
recon_comp_mag_psnr = psnr(abs(img_3d(:,:,ind_test_st)),double(recon_mag_comp(:,:,1)))
recon_comp2_mag_psnr = psnr(abs(img_3d(:,:,ind_test_st)),double(recon_mag_comp2(:,:,1)))
...
recon_comp_phase_psnr = psnr(angle(img_3d(:,:,ind_test_st)),double(recon_phase_comp(:,:,1)))
recon_comp2_phase_psnr = psnr(angle(img_3d(:,:,ind_test_st)),double(recon_phase_comp2(:,:,1)))
recon_lp_phase_psnr = psnr(angle(img_3d(:,:,ind_test_st)),double(recon_phase_lp(:,:,1)))